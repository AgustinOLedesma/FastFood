--Nos popsicionamos en la base de datos para empezar a trabajar
use FastFoodDB
...
--Eliminamos los pedidos que anteriormente nos piden

DELETE FROM Productos WHERE CategoriaID = 2;

-- Restaurarmos la restricción de clave externa
...
SELECT COUNT(DISTINCT OrdenID) AS TotalRegistrosUnicos
FROM Ordenes;
...
SELECT Departamento, 
	COUNT(EmpleadoID) CantidadEmpleados
FROM Empleados 
...
SELECT CategoriaID,
	COUNT(ProductoID) CantidadProductos
FROM Productos
...
Pregunta: ¿Cuáles son las sucursales con un promedio de ventas por orden superior a un valor específico (40), 
